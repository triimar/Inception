...
if [ ! -d "/var/lib/mysql/wordpress" ]; then
	...
	mysql -u root -p"$(cat ${MYSQL_ROOT_PASSWORD_FILE})" -e "CREATE USER IF NOT EXISTS '${MYSQL_USER}'@'%' IDENTIFIED BY '$(cat ${MYSQL_PASSWORD_FILE})';"
	mysql -u root -p"$(cat ${MYSQL_ROOT_PASSWORD_FILE})" -e "GRANT ALL PRIVILEGES ON ${MYSQL_DATABASE}.* TO '${MYSQL_USER}'@'%';"
